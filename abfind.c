...
void abfind(int nell,double *r1,double *u,struct common2 *elem1,float *xpt,float *ypt,struct common4 *sparse,struct common5 *param,int *ncod,int *indexAA)
{
    double phi[9],phic[9],phie[9],usol=0,usol1=0,usol2=0,x1=0,x2=0,y1=0,y2=0,dett=0;
    int ngl[9];
    double tphx[9],tphy[9],value=0;
    double rest=0,estifm[9][9];
    ...
    for (t=0; t<9; t++) {
        for (r=0; r<9; r++) {
            ...
            p=ngl[t];
            q=ngl[r];
            CSR(sparse,value,p,q,ncod,indexAA);
        }
    }
